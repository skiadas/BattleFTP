...
{-
    ...
    Takes a Unit and an action and calculates the amount of damage (or healing) dealt by the source
        which could be a hero or enemy, it just depends on which unit is taking the action
-}
...
{-
    Given a Source Unit, a Target Unit and some action (defined in ActionOptions.hs), update the Target's health
    Use the actionValue function to determine the effect based off of the unit's attributes & equipped weapons
    Unit should be keeping track of how inventory affects unit attributes, so Combat module does not need to worry
        about how inventory affects units. Similarly, Game & State keeps track of up-to-date stats (primariliy health)
        so Combat just needs access to that current information. 
-}
